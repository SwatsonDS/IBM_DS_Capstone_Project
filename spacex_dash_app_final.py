...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        #print(filtered_df)  
        fig = px.pie(filtered_df,values='class count',names='class', title='Total Successful Launches for site '+str(entered_site))  
        return fig
...
def get_scatter_chart(entered_site,payload_slider):
    ...
    if entered_site == 'ALL':
        filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] < payload_slider[1]) & (spacex_df['Payload Mass (kg)'] > payload_slider[0])]
        print('***Slider min:',payload_slider[0],' Slider max:',payload_slider[1])
        ...
        print('***Site:',entered_site,'Slider min:',payload_slider[0],' Slider max:',payload_slider[1])
        fig = px.scatter(filtered_df,x='Payload Mass (kg)',y='class',
                         color='Booster Version Category',title='Correlation between Payload and Success for '+str(entered_site))  
        return fig
